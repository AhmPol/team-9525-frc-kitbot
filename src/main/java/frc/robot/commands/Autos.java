...
public final class Autos {
  ...
  public static Command exampleAuto(CANDrivetrain drivetrain) {
     ...
     */
    return new RunCommand(() -> drivetrain.arcadeDrive(AutoConstants.kAutoDrivePower, 0), drivetrain)
        .withTimeout(AutoConstants.kAutoTimeout)
        .andThen(new RunCommand(() -> drivetrain.arcadeDrive(0, 0), drivetrain));
  }

    /** Example static factory for an autonomous command. */
    public static Command simpleAuto(CANDrivetrain drivetrain) {
      return new RunCommand(() -> drivetrain.arcadeDrive(AutoConstants.kAutoDrivePower, 0), drivetrain)
          .withTimeout(AutoConstants.kAutoTimeout)
          .andThen(new RunCommand(() -> drivetrain.arcadeDrive(0, 0), drivetrain));
    }

  public static Command shootAndSitAuto(CANLauncher launcher) {
  ...
  public static Command shootAndDriveDiagonalBackwardAuto(CANDrivetrain drivetrain, CANLauncher launcher) {
    ...
    public static Command shootAndDriveStraightBackwardAuto(CANDrivetrain drivetrain, CANLauncher launcher) {
    ...
    return Commands.sequence(
      ...
      .withTimeout(1.3*AutoConstants.kAutoTimeout)
      // .andThen(new RunCommand(() -> drivetrain.arcadeDrive(0.75, 0), drivetrain))
