...
import frc.robot.commands.Auto.ShootandSit;
...
public class RobotContainer {
  ...
  private final VisionSystem cameraSubsystem = new VisionSystem();
  ...
  private final Command RedRightAuto = new RedRightAuto(drivetrain, launcher); 
  private final Command RedLeftAuto = new RedLeftAuto(drivetrain, launcher); 
  private final Command BlueRightAuto = new BlueRightAuto(drivetrain, launcher);
  private final Command BlueLeftAuto = new BlueLeftAuto(drivetrain, launcher); 
  private final Command ShootandSit = new ShootandSit(drivetrain, launcher); 
   ...
   */
  // Inside the constructor of RobotContainer
  ...
  public RobotContainer() {
    ...
    chooser.addOption("ShootandSit", ShootandSit);
    chooser.addOption("BlueLeftAuto", BlueLeftAuto);
    chooser.addOption("BlueRightAuto", BlueRightAuto);
    
    // Put the chooser data onto SmartDashboard
    SmartDashboard.putData("Auto choices", chooser);
    
    // Display match time on SmartDashboard
    
    // Display the vision system connection status
    //vision.updateCameraStatus();

    // Put subsystems onto Shuffleboard (as an alternative to SmartDashboard)
